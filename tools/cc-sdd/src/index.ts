...
import { type CliIO, defaultIO } from "./cli/io.js";
import {
	type CategoryPolicyMap,
	determineCategoryPolicies,
	printSummary,
	summarizeCategories,
} from "./cli/policies.js";
import {
	colors,
	formatBox,
	formatError,
...
import {
	buildFileOperations,
	type FileOperation,
} from "./plan/fileOperations.js";
import { formatProcessedArtifacts } from "./plan/printer.js";

const agentKeys = agentList;
const aliasFlags = Array.from(
	new Set(agentKeys.flatMap((key) => getAgentDefinition(key).aliasFlags)),
);

const agentAliasLine =
	aliasFlags.length > 0
		? `  ${aliasFlags.join(" | ")}  Agent alias flags\n`
		: "";
	...
	return async (info: ConflictInfo): Promise<ConflictDecision> => {
		...
		const choices: {
		...
		}[] = [
			{
				...
				description: "Replace with the latest template content.",
			},
			{
				value: "skip",
				label: "Keep existing file",
				description: "Leave the current file unchanged.",
			},
		];

		if (
			info.category === "project-memory" &&
			info.sourceMode !== "template-json"
		) {
			choices.splice(1, 0, {
				value: "append",
		...
		const decision = await promptChoice(
			promptMessage,
			choices,
			Math.min(defaultIndex, choices.length - 1),
		);

		if (remaining > 1) {
			const applyToRest = await promptConfirm(
				"Apply this choice to remaining files in this category?",
				true,
			);
...
const handleDryRun = async (
	...
	try {
		...
		const operations = await buildFileOperations(
			plan,
			resolvedConfig,
			execOpts,
		);
...
const runPlanExecution = async (
	...
	try {
		...
		const conflictHandler = createConflictHandler(
			summaries,
			resolvedConfig.effectiveOverwrite,
		);
		if (!conflictHandler && resolvedConfig.effectiveOverwrite === "prompt") {
			io.log(
				formatWarning(
					"Prompt mode unavailable; existing files will be skipped. Use --yes or --overwrite=force.",
				),
			);
		}
		...
		io.log(
			formatSuccess(`  ${result.written}/${total} files written`) +
				(result.skipped > 0
					? colors.yellow(`, ${result.skipped} skipped`)
					: ""),
		);
		io.log("");
...
export const runCli = async (
	...
	parsedArgs.agent = await ensureAgentSelection(
		parsedArgs.agent ?? loadedConfig.agent,
		io,
	);
	...
	if (parsedArgs.agent === "codex") {
		...
		io.log("");
		io.log(
			`  Codex no longer loads ${colors.dim(".codex/prompts/")}. Use Skills instead:`,
		);
		io.log("");
