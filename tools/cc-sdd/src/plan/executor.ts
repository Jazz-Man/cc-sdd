...
const backupIfNeeded = async (
	...
	const backupRoot = assertPathInsideRoot(
		path.resolve(cwd, resolved.backupDir),
		cwd,
		"Backup directory",
	);
	const backupPath = assertPathInsideRoot(
		path.resolve(backupRoot, rel),
		backupRoot,
		"Backup path",
	);
...
const appendContent = async (
	...
	await appendFile(
		target,
		(await fileExists(target)) ? `${separator}${normalized}` : normalized,
		"utf8",
	);
};
...
const handleAppend = async (
	...
	if (!canAppend(op.sourceMode)) {
		opts.log?.(
			`Append not supported for ${op.relTarget} (non-text content). Skipping.`,
		);
		return false;
	}
	const safeDest = assertPathInsideRoot(op.destAbs, cwd, "Destination path");
...
export const executeProcessedArtifacts = async (
	...
	const operations =
		opts.operations ?? (await buildFileOperations(items, resolved, opts));
	let written = 0;
	let skipped = 0;
	...
	for (const op of operations) {
		...
		if (action === "append" && !canAppend(op.sourceMode)) {
			opts.log?.(
				`Append not supported for ${op.relTarget} (non-text content). Overwriting instead.`,
			);
			action = "write";
		}
