...
import { buildSharedRuleOperations, parseSharedRules } from "./sharedRules.js";
...
const transformTemplateOutput = (
	relPath: string,
): { outName: string; mode: "json" | "text" } => {
	const dirName = path.dirname(relPath);
	const base = path.basename(relPath);
	if (base.endsWith(".tpl.json")) {
		const replaced = `${base.slice(0, -".tpl.json".length)}.json`;
		return {
			outName: dirName === "." ? replaced : path.join(dirName, replaced),
			mode: "json",
		};
	}
...
export const buildFileOperations = async (
	...
	for (const art of artifacts) {
		if (art.source.type === "staticDir") {
			const srcDir = resolveRelativePathInsideRoot(
				templatesRoot,
				art.source.from,
				`Artifact ${art.id} source path`,
			);
			const destDir = resolveRelativePathInsideRoot(
				cwd,
				art.source.toDir,
				`Artifact ${art.id} destination path`,
			);
			...
			for (const src of files) {
				...
				const destAbs = assertPathInsideRoot(
					...
					cwd,
					`Artifact ${art.id} destination file`,
				);
		...
		if (art.source.type === "templateFile") {
			const srcAbs = resolveRelativePathInsideRoot(
				templatesRoot,
				art.source.from,
				`Artifact ${art.id} source path`,
			);
			...
			const destAbs = assertPathInsideRoot(
				path.join(destDir, art.source.outFile),
				cwd,
				`Artifact ${art.id} destination file`,
			);
			const relTarget = path.relative(cwd, destAbs);
			...
			const fallbackRel =
				agentDefinition.templateFallbacks?.[art.source.outFile];
			operations.push({
				artifactId: art.id,
				srcAbs,
				destAbs,
