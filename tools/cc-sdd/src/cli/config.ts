...
export const mergeConfigAndArgs = (
	...
	const agent: AgentType = (args.agent ??
		config.agent ??
	...
	const kiroDir = resolveKiroDir({
		flag: args.kiroDir,
		config: config.kiroDir,
	});

	const overwrite: OverwritePolicy = (args.overwrite ??
		config.overwrite ??
		defaults.overwrite) as OverwritePolicy;
	const yes = !!args.yes;
	const effectiveOverwrite: OverwritePolicy =
		yes && overwrite === "prompt" ? "force" : overwrite;
	...
	const layout = resolveAgentLayout(agent, {
		agentLayouts: config.agentLayouts,
	});
