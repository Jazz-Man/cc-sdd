...
export type CategoryPolicyMap = Partial<
	Record<InstallCategory, "inherit" | "overwrite" | "skip" | "append">
>;

export const summarizeCategories = async (
...
const hasConflicts = (summary: CategorySummary): boolean =>
	summary.existing > 0;
			...
			[
				{
					...
					description:
						"Replace the document with the latest template (previous content will be backed up if enabled).",
				},
				{
					value: "append",
					label: `Append updates to ${docName}`,
					description:
						"Keep your existing notes and add new template sections after them.",
				},
				{
					value: "skip",
