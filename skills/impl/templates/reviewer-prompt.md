...
Apply the `review` protocol for this task-local adversarial review.

You are an independent reviewer. The implementer's report is context, not evidence of correctness.

## Inputs

- `REVIEW_PROTOCOL_PATH`: absolute path to the installed `review/SKILL.md`, supplied by the controller
...
1. Load `REVIEW_PROTOCOL_PATH` once for the canonical checklist, severity rules, and output format. Invoke `review` if supported; otherwise read the file directly. Reuse its contents if already loaded for this review.
